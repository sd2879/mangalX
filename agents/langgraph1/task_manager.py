...
from agents.langgraph.brave_search import BraveSearchAgent  # [CHANGE: Added import]
...
class AgentTaskManager(InMemoryTaskManager):
    ...
    def _validate_request(
        ...
        task_send_params: TaskSendParams = request.params
        # [CHANGE: Modified validation to support both agents' content types]
        supported_types = list(set(CurrencyAgent.SUPPORTED_CONTENT_TYPES + BraveSearchAgent.SUPPORTED_CONTENT_TYPES))
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, supported_types
        ):
            logger.warning(
...
# [CHANGE: Added MultiAgentTaskManager]
class MultiAgentTaskManager(InMemoryTaskManager):
    ...
    def _select_agent(self, query: str):
        """Selects the appropriate agent based on query content."""
        if "search for" in query.lower():
            return self.brave_search_agent
        return self.currency_agent
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        agent = self._select_agent(query)
        try:
            agent_response = agent.invoke(query, task_send_params.sessionId)
        except Exception as e:
            logger.error(f"Error invoking agent: {e}")
            raise ValueError(f"Error invoking agent: {e}")
        return await self._process_agent_response(
